...
('read_file_tail', '读取文件结尾', '读取允许目录下文本文件的最后 N 行，常用于查看日志。', 'python_builtin', 'all', '', '', '[]', '{"type":"object","properties":{"path":{"type":"string"},"lines":{"type":"integer","minimum":1,"maximum":200,"default":50},"encoding":{"type":"string","default":"utf-8"}},"required":["path"]}', '["*"]', 1, 0, 'low', 10, 32768, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('find_files', '查找文件', '在允许目录下按文件名或通配符查找文件。', 'python_builtin', 'all', '', '', '[]', '{"type":"object","properties":{"path":{"type":"string"},"pattern":{"type":"string"},"max_results":{"type":"integer","minimum":1,"maximum":200,"default":50}},"required":["path","pattern"]}', '["*"]', 1, 0, 'low', 15, 65536, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('search_text', '搜索文本', '在允许目录下按关键字搜索文本，优先由后端使用 rg 或平台原生命令实现。', 'python_builtin', 'all', '', '', '[]', '{"type":"object","properties":{"path":{"type":"string"},"pattern":{"type":"string"},"case_sensitive":{"type":"boolean","default":false},"max_results":{"type":"integer","minimum":1,"maximum":100,"default":20}},"required":["path","pattern"]}', '["*"]', 1, 0, 'medium', 15, 65536, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
...
('get_directory_size', '查看目录大小', '统计允许目录下指定目录的大小。', 'python_builtin', 'all', '', '', '[]', '{"type":"object","properties":{"path":{"type":"string"},"max_depth":{"type":"integer","minimum":0,"maximum":5,"default":3}},"required":["path"]}', '["*"]', 1, 0, 'medium', 30, 32768, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
